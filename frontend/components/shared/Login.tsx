...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Login submitted", { email, password, username });
  };
  ...
  const socialButtons = [
    {
      ...
      color: "text-gray-800 hover:bg-gray-800 hover:text-white",
    },
    {
      name: "Google",
      icon: (
      ...
      <div className="w-full max-w-md bg-white md:p-8 p-4  rounded-lg shadow-lg transform transition-all">
        ...
        <div className="flex flex-col space-y-4 mb-4">
          {socialButtons.map(({ name, icon, color }) => (
            <button
              key={name}
              className={`flex items-center justify-center px-3 py-2 border border-gray-300 rounded-md text-sm font-medium bg-white ${color}`}
            >
